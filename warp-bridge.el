;;; warp-bridge.el --- Worker Connection and State Management -*- lexical-binding: t; -*-

;;; Commentary:
;;
;; This module provides the master-side "bridge" for managing raw
;; worker connections. Its role is narrowly focused on the transport
;; layer, initial worker handshake, and maintaining the authoritative
;; state of each worker in the shared `state-manager`.
;;
;; ## Key Responsibilities:
;;
;; - **Transport Management**: Opens and manages the listening endpoint
;;   (e.g., TCP) for incoming worker connections.
;; - **Secure Handshake**: Verifies the cryptographic handshake from new
...
;;   master-bound RPCs, delegating general requests to the main RPC system.
...
(defun warp-bridge--deregister-worker (bridge worker-id)
  "Completely remove a worker and its services from the state manager.
    ...
    (braid!
     ...
     (:then
      (lambda (_)
        ...
        ;; (e.g., health orchestrator) can perform their own cleanup.
...
(defun warp-bridge--update-worker-health (bridge worker-id new-status)
  "Update a worker's health status and trigger deregistration if needed.
This is a reactive event handler, called in response to a
...
Side Effects:
- Updates the worker's record in the state manager.
...
(defun warp-bridge--handle-worker-ready (bridge message connection)
  "Handle the initial `:worker-ready` RPC and its security handshake.
This is the gatekeeper for new workers. It verifies the worker's one-time
launch token and signature. If successful, it creates the authoritative
`warp-managed-worker` record in the state manager.

Arguments:
- `BRIDGE` (warp-bridge): The bridge instance.
- `MESSAGE` (warp-rpc-message): The full RPC message from the worker.
- `CONNECTION` (warp-transport-connection): The worker's connection.
...
Side Effects:
- Verifies the launch token; closes connection on failure.
- Creates a new worker record in the state manager on success.
- Registers the worker's IPC address on success.
- Emits a `:worker-ready-signal-received` event on success."
  (braid!
   (let* ((args (warp-rpc-message-payload message))
   ...
   (:then
    (lambda (_)
      (let* ((args (warp-rpc-message-payload message))
        ...
        ;; If verification passes, create the managed worker object for
        ;; the state manager, establishing it as a known cluster member.
        (let ((m-worker
...
Returns:
- (any): The payload stored for the worker's rank, or `nil`."
  (let* ((sm (warp-bridge-state-manager bridge))
...
- Subscribes handlers to the `:worker-status-changed` and
  ...
  (let ((bridge (%%make-warp-bridge
    ...
    ;; orchestrator) to maintain the authoritative health state.
    (warp:subscribe
     ...
     (lambda (event)
       (let* ((sm (warp-bridge-state-manager bridge))
              ...
              (new-endpoint (make-warp-service-endpoint
                             ...
                             :address (plist-get data :address)))) ; TODO: Check this, believe this isn't currently correct
...
(defun warp:bridge-start (bridge)
  "Initialize and start the transport layer listener.
This function brings the bridge online by opening a network socket (or
other transport) to listen for incoming connections from workers.
...
- Closes the main transport server socket."
  (when-let ((server (warp-bridge-transport-server bridge)))
    ...
    (ignore-errors (warp:transport-close server)))
  (loom:resolved! t))

;;;###autoload
(defun warp:bridge-get-contact-address (bridge)
  "Get the master's contact address for new workers to connect to.
...
sequence to perform custom, rank-specific initialization.
