...
;; It defines the core responsibilities and behaviors of a worker process
;; operating within a distributed cluster managed by a Warp leader.
;;
...
;; improves modularity, testability, and maintainability, allowing for
;; complex worker functionality to be assembled from reusable building blocks.
;;
;; ## Key Design Principles:
;;
;; - **Declarative Assembly**: The worker's entire object graph, from
;;   the connection manager to the heartbeat service, is defined
;;   declaratively within `warp-worker--get-component-definitions`. The
;;   component system handles the complex task of instantiating and
;;   wiring everything together in the correct order, resolving all
;;   dependencies automatically.
;;
;; - **Dynamic Leader Discovery**: The worker no longer connects to a static
;;   master address. Instead, it dynamically discovers the active leader
;;   via the coordinator cluster (`warp-coordinator.el`) and will
;;   automatically reconnect to a new leader upon failover. This ensures
...
;;   orchestrator, using the enhanced `warp:defconfig` system. This makes
;;   deployment flexible and cloud-native friendly.
;;
;; - **Integrated Distributed Tracing**: The worker is fully instrumented
;;   for **distributed tracing**. It sets up a trace exporter and uses an
;;   RPC middleware to automatically capture and propagate trace contexts
;;   for all incoming RPC requests, enabling end-to-end visibility across
;;   distributed operations.
;;
;; - **Remote Code Execution Security**: Integrates `warp-security-policy.el`
;;   to safely execute dynamic Lisp code received from the leader, with
;;   configurable security levels and authentication.
;;
;; - **Managed Resource Pools**: Utilizes `warp-executor-pool.el` for
;;   managing sandboxed execution of tasks, isolating application logic
;;   from the core worker runtime.
...
This is a critical startup error, as the worker cannot connect to its
control plane."
  'warp-worker-error)
...
stage, preventing worker overload."
  'warp-worker-error)
...
process from those in sandboxed sub-processes, aiding debugging."
  'warp-worker-error)
...
errors during the initial secure handshake with the leader, potentially
preventing the worker from joining the cluster securely."
  'warp-worker-error)
...
functions and shutdown hooks. It's set by `warp:worker-create`."
  :type '(or null warp-worker))

(defvar warp--worker-profiles (make-hash-table :test 'eq)
  "A global registry for named worker configuration profiles.
Allows defining reusable templates for different worker types (e.g.,
'etl-worker', 'map-reduce-worker') to simplify cluster configuration."
  :type 'hash-table)
...
making it cloud-native friendly.

Fields:
- `heartbeat-interval` (float): Interval in seconds between sending
  liveness and metrics heartbeats to the control plane leader.
- `max-concurrent-requests` (integer): Maximum number of incoming RPC
  requests this worker will process concurrently. Used for backpressure
  to prevent overload.
- `request-timeout` (float): Default timeout in seconds for processing
  an incoming RPC request. If a request exceeds this time, it's failed.
- `enable-job-processing` (boolean): If `t`, this worker will actively
  fetch and execute jobs from the distributed `warp-job-queue`.
- `job-queue-worker-id` (string): The logical ID of the `warp-job-queue`
  worker instance from which this worker should fetch jobs.
- `security-level` (symbol): Default security policy for evaluating
  dynamic code received from the leader (`:strict`, `:permissive`,
  `:unrestricted`). Controls sandboxing severity.
- `security-strategy` (keyword): Strategy for provisioning cryptographic
  keys (`:offline-provisioning` or `:master-enrollment`). Determines how
  the worker obtains its keys for secure communication.
- `metrics-interval` (float): Interval in seconds for collecting internal
  system metrics (CPU, memory, active requests) via `warp-system-monitor`.
- `shutdown-timeout` (float): Maximum time in seconds to wait for active
  requests to complete during a graceful shutdown. If exceeded, shutdown
  proceeds forcefully.
- `rank` (integer or nil): The numerical rank assigned by the master,
  typically indicating its position in a homogeneous pool (e.g., 0, 1, 2).
  Loaded from environment variable `WARP_WORKER_RANK`.
- `coordinator-peers` (list or nil): A list of peer addresses (strings)
  for the `warp-coordinator` cluster. Used by this worker for initial
  leader discovery to connect to the control plane. Loaded from `WARP_COORD_PEERS`.
- `control-plane-transport-options` (plist or nil): Options for the
  client-side `warp-transport` connection to the control plane leader
  (e.g., SSL settings, custom serializers).
- `log-level` (symbol): Default logging verbosity level for this worker
  (`:debug`, `:info`, `:warn`, `:error`, `:fatal`).
- `trace-log-file-path` (string or nil): If set, enables distributed
  tracing for this worker and specifies the file to which completed
  trace spans are written as JSON.
- `rate-limit-enabled` (boolean): If non-nil, enables a global rate
  limiter for incoming RPC requests, preventing overload.
- `rate-limit-max-requests` (integer): Maximum number of requests allowed
  within the `rate-limit-window-seconds`.
- `rate-limit-window-seconds` (integer): Time window (in seconds) for
  the rate limit.
- `health-check-interval` (float): Interval in seconds for running internal
  health checks (CPU, memory, queue depth) that contribute to the worker's
  reported health status.
- `memory-threshold-mb` (integer): Memory usage in MB that, if
  exceeded, marks the worker's health as `:degraded`.
- `cpu-degradation-threshold` (float): CPU utilization percentage
  (0.0-100.0) above which the worker's health is reported as `:degraded`.
- `cpu-unhealthy-threshold` (float): CPU utilization percentage above
  which the worker's health is reported as `:unhealthy`. Must be higher
  than `cpu-degradation-threshold`.
- `queue-degradation-ratio` (float): Ratio (0.0-1.0) of active requests
  to `max-concurrent-requests` above which health becomes `:degraded`.
- `queue-unhealthy-ratio` (float): Ratio of active requests to max
  concurrent requests above which health becomes `:unhealthy`. Must be
  higher than `queue-degradation-ratio`.
- `processing-pool-min-size` (integer): Minimum number of sub-processes
  or threads to keep alive in the `executor-pool`.
- `processing-pool-max-size` (integer): Maximum number of sub-processes
  or threads to scale up to in the `executor-pool`.
- `processing-pool-idle-timeout` (integer): Seconds after which an idle
  sub-process in the `executor-pool` is terminated to free resources.
- `processing-pool-request-timeout` (float): Timeout for a single task
  executed within the `executor-pool`.
- `is-coordinator-node` (boolean): If `t`, this worker also runs a
  `warp-coordinator` instance for distributed consensus, making it part
...
- `total-requests-processed` (integer): Cumulative counter for all
  successfully processed RPC requests since worker startup.
- `active-request-count` (integer): Number of RPCs currently being
  processed by the worker. Key metric for real-time load and
  backpressure.
...
- `registered-service-count` (integer): Number of dynamically registered
  services hosted by this worker (e.g., custom RPC commands).
- `circuit-breaker-stats` (hash-table): Aggregated state of any active
  `warp-circuit-breaker` instances managed by this worker (e.g., 'open',
  'half-open', 'closed').
...
- `health-status` (symbol): Overall health (`:healthy`, `:degraded`,
  `:unhealthy`) as determined by the `health-orchestrator`."
...
which manages all of its dynamic runtime state and subsystems. It acts
as the context for all worker-specific operations.

Fields:
- `worker-id` (string): A unique identifier for this worker process.
  This ID is stable across restarts and is used for identification in
  the cluster.
- `rank` (integer or nil): An optional numerical rank assigned by the master,
  used for scheduling or priority within a worker pool.
- `startup-time` (float): The `float-time` when the worker was created.
  Used for calculating uptime and various metrics.
- `config` (worker-config): The immutable, but hot-reloadable, configuration
  object for this worker. It defines all its operational parameters.
- `component-system` (warp-component-system): The heart of the worker's
  ...
  (start, stop, dependencies). This provides the worker's full functionality.
- `shutdown-hooks` (list): User-provided functions to be executed during
  graceful shutdown for custom application cleanup. These allow for
  application-specific resource release before core worker components terminate."
...
(defun warp-worker--discover-leader-address (system config)
  "Dynamically discovers the current leader's contact address from coordinator peers.
This function iterates through the configured `coordinator-peers`, attempting
to establish a temporary connection to each. It then sends an RPC request
to the connected peer's `warp-coordinator` component, asking for the active
leader's contact address. This ensures the worker can find and reconnect to
the control plane leader even after failovers.
...
- (loom-promise): A promise that resolves with the leader's address string
  (e.g., \"tcp://leader.example.com:8080\") on success, or rejects with
  `warp-worker-leader-discovery-failed` if no leader can be found after
  trying all peers.

Side Effects:
- Logs discovery attempts and success/failure.
- Establishes transient connections to coordinator peers."
  (let* ((worker-id (warp-worker-worker-id (warp:component-context-get :worker)))
         ...
         (connection-manager (warp:component-system-get system :connection-manager))
    ...
    (loom:loop!
      (let ((peer (pop peers)))
        (unless peer
          ;; If all configured peers have been tried and failed, reject the promise.
          (loom:break!
            (warp:error!
             :type 'warp-worker-leader-discovery-failed
             :message "Could not discover leader from any configured coordinator peer after multiple attempts.")))

        (warp:log! :info worker-id "Attempting leader discovery via peer: %s" peer)

        ;; Attempt to connect to the peer.
        (braid! (warp:transport-connect peer :options transport-options)
          (:then (peer-conn)
            (warp:log! :debug worker-id "Connected to peer %s for leader discovery." peer)
            (unwind-protect
                (braid! (warp:protocol-get-coordinator-leader rpc-system peer-conn worker-id peer) ; NEW RPC
                  (:then (leader-address)
                    (if leader-address
                        ;; Success! We found the leader address. Break the loop and return it.
                        (progn
                          (warp:log! :info worker-id "Discovered leader at: %s." leader-address)
                          (loom:break! leader-address))
                      ;; Peer responded but had no leader info. Try next peer.
                      (warp:log! :warn worker-id "Peer %s did not provide leader info. Trying next..." peer)
                      (loom:continue!)))
                  (:catch (rpc-err)
                    ;; RPC to peer's coordinator failed. Log and try the next one.
                    (warp:log! :warn worker-id "RPC to peer %s failed during leader discovery. Trying next... (Error: %S)" peer rpc-err)
                    (loom:continue!)))
              ;; Ensure connection is closed after discovery attempt.
              (warp:transport-close peer-conn)))
          (:catch (conn-err)
            ;; This peer is likely down or unreachable. Log and try the next one.
            (warp:log! :warn worker-id "Failed to connect to peer %s for leader discovery. Trying next... (Error: %S)" peer conn-err)
            (loom:continue!))))))))
...
`warp-component` system uses this list to build the worker's runtime
...
Returns:
- (list): A list of plists, where each plist defines a component for the
  worker's component system. Each definition includes `:name`, `:deps`,
  `:factory`, `:start`, `:stop`, and optional `:priority`."
  (append
   (list
    ;; --- Core Infrastructure Components (Always Present) ---

    ;; `ipc-system`: Handles all inter-process and inter-thread messaging
    ;; within this Emacs instance. Essential for internal communication
    ;; between components (e.g., RPC system and command router) and
    ...
    ;; `trace-exporter`: Exports completed distributed trace spans to a
    ;; configurable backend (e.g., a file). This is part of the worker's
    ;; observability stack, enabling end-to-end tracing of requests.
    `(:name :trace-exporter
      :factory (lambda ()
                 (when-let (log-file (worker-config-trace-log-file-path config))
    ...
    ;; connection status changes, health check updates, state manager events).
    ;; Other components can subscribe to or emit events via this bus.
    `(:name :event-system
      :factory (lambda ()
    ...
    ;; machine, as well as for other shared state that needs to be consistent
    ...
    ;; and reporting to the master as part of the worker's heartbeat.
    `(:name :system-monitor
      :factory (lambda ()
    ...
    ;; for executing arbitrary Lisp code received via RPC. This isolates task
    ;; execution from the main worker process, enhancing stability and security.
    `(:name :executor-pool
      :factory (lambda ()
                 (warp:executor-pool-create
                  :name ,(format "%s-pool" (warp-worker-worker-id worker))
                  :min-size (worker-config-processing-pool-min-size config)
                  :max-size (worker-config-processing-pool-max-size config)
                  :idle-timeout (worker-config-processing-pool-idle-timeout config)
                  :request-timeout (worker-config-processing-pool-request-timeout config)))
      :stop (lambda (pool _) (loom:await (warp:executor-pool-shutdown pool))))
    ...
    `(:name :connection-manager
      :deps (:request-pipeline :rpc-system :event-system :coordinator) ; Added :coordinator for leader discovery
      :factory (lambda (pipeline rpc-system event-system coordinator)
      ...
      :start (lambda (cm _) (loom:await (warp:connection-manager-start cm)))
    ...
    ;; Lisp evaluation, preventing malicious code execution and data access.
    ...
    ;; It uses the `security-policy` to verify sender identity and message
    ;; integrity (e.g., JWT validation).
    ...
    ;; with too many requests, maintaining stability under high load.
    ...
    ;; RPC requests. It's a chain of middleware that includes unpacking,
    ;; authentication, rate limiting, and finally dispatching the command to its
    ;; handler. This ensures a consistent and controlled request flow.
      ...
      :factory (lambda (router auth-mw limiter-mw)
                 (warp:request-pipeline-create
                  :stages (list (warp:request-pipeline-stage
                                  ...
                                  ;; from the raw message and sets it in context.
                                  (lambda (context)
    ...
    ;; by the worker at runtime, enabling a pluggable service architecture.
    ...
    ;; (like log level, heartbeat interval) without requiring a full worker
    ;; restart, enhancing operational flexibility.
    ...
    ;; registered health checks (internal and potentially external). It
    ;; determines the worker's overall health status based on individual
    ;; check outcomes, which is then reported to the master.
    ...
    ;; (CPU, memory, event queue depth, etc.) with the `health-orchestrator`.
    ...
    ;; secure handshake with the master/leader. It announces the worker's presence
    ;; and securely registers itself with the control plane, including sending
    ;; launch tokens and signatures.
    `(:name :ready-signal-service
      ...
      :start (lambda (_ system) (loom:await (warp-worker--send-ready-signal worker system))))

    ;; `init-payload-service`: A 'run-once' service that requests an initial
    ;; Lisp form (payload) to execute from the master. This allows dynamic
    ;; provisioning of worker-specific setup code upon startup (e.g., loading
    ;; specialized application logic or configuration). The payload is executed
    ;; within the worker's security policy.
    ...
    ;; intelligent load balancing, health monitoring, and dynamic scaling.
    ...
    ;; This `rpc-service` is the actual RPC server for the worker,
    ;; registering handlers for commands the master or other components
    ;; might send *to this worker*. This is often a generic RPC server
    ;; used by many components, but listed here for clarity.
    `(:name :rpc-service
      :deps (:command-router :executor-pool) ; Depends on router for dispatch, and executor for task handling.
      :start (lambda (_ system)
               (let ((router (warp:component-system-get system :command-router)))
                 (warp:defrpc-handlers router
                   ...
                   (:execute-job . #'warp-handler-execute-job))))))
   ...
   (when (worker-config-enable-job-processing config)
     (list
      ...
      `(:name :job-processor-service
        :deps (:connection-manager :rpc-system) ; Needs RPC for communication with job queue worker.
...
attempts to fetch a job from the distributed job queue (via RPC to the
Job Queue Worker), executes it by sending an RPC command to itself
(which then goes through the request pipeline and executor pool for
sandboxed execution), and reports the job's completion or failure back
to the Job Queue Worker.
...
- Updates worker metrics related to job processing.
- Logs job lifecycle events."
  (let* ((worker-id (warp-worker-worker-id worker))
         ...
         (service-client (warp:component-system-get system :service-client)) 
         (jq-worker-id (worker-config-job-queue-worker-id config))
         (jq-endpoint (loom:await (warp:service-client-get-endpoint service-client "job-queue" :worker-id jq-worker-id))) 
         (jq-worker-address (and jq-endpoint (plist-get jq-endpoint :address))) 
         (conn (warp:connection-manager-get-connection cm))) ; Assumes single master conn

    (when (and conn jq-worker-address) ;; Ensure we have a master connection AND JQ worker address
      (braid!
        ;; 1. Fetch a job from the queue. Send an RPC request to the job queue worker.
        (let ((command (make-warp-rpc-command :name :job-fetch)))
          (warp:log! :debug worker-id "Requesting job from %s..." jq-worker-id)
          (warp:rpc-request rpc-system conn worker-id jq-worker-id command))

        (:then (lambda (job)
                 (when job
                   (warp:log! :info worker-id "Received job %s to execute (payload: %S)."
                              (warp-job-id job) (warp-job-payload job))
                   ;; 2. Execute the job by making an RPC call to *itself*.
                   ;; This is a critical pattern: it routes the job execution
                   ;; through the worker's own request pipeline, ensuring
                   ;; that security policies, rate limiting, and the executor
                   ;; pool are applied to the job's payload.
                   (let ((exec-cmd (make-warp-rpc-command :name :execute-job
                     ...
                     (braid! (warp:rpc-request rpc-system conn worker-id worker-id exec-cmd)
                       ;; 3. Report job completion back to the job queue worker.
                       (:then (lambda (result)
                                (warp:log! :info worker-id "Job %s completed successfully." (warp-job-id job))
                                (let ((comp-cmd (make-warp-rpc-command
                                  ...
                                  (warp:rpc-request rpc-system conn worker-id jq-worker-id comp-cmd
                                                    :expect-response nil)))) ; No response expected for completion report.
                       ;; 4. Report job failure back to the job queue worker.
                       (:catch (lambda (err)
                                 (warp:log! :error worker-id "Job %s failed: %S" (warp-job-id job) err)
                                 (let ((fail-cmd (make-warp-rpc-command
                                   ...
                                   (warp:rpc-request rpc-system conn worker-id jq-worker-id fail-cmd
                                                     :expect-response nil)))))))))
        (:catch (lambda (err)
                  ;; This catch handles errors in fetching the job itself.
                  (warp:log! :error worker-id "Error in job processing loop (fetching job): %S" err)))
        (:finally (lambda ()
                    ;; Reschedule the job processing loop for the next iteration.
                    ;; This ensures continuous job fetching as long as the worker is alive.
                    (let ((holder (warp:component-system-get system :job-processor-service)))
                      (when (timerp (gethash :timer holder)) ; Check if not shutting down
                        (puthash :timer (run-at-time 1 nil #'warp-worker--job-processing-loop worker system)
                                 holder)))))))))
  (unless jq-worker-address
    (warp:log! :warn worker-id "Job Queue Worker endpoint not found. Job processing loop paused.")))
...
This function is called once at startup (by the `ready-signal-service` component).
It announces the worker's presence and performs the initial secure handshake
with the master/leader. This is a critical step for the worker to join the
cluster. It sends worker details, launch tokens, and cryptographic signatures.
...
Returns:
- (loom-promise): A promise that resolves when the signal is successfully
  sent and acknowledged by the leader. Rejects on connection or security errors.

Side Effects:
- Performs cryptographic signing of worker data.
- Sends an RPC request (`:send-worker-ready`) to the leader."
  (let* ((cm (warp:component-system-get system :connection-manager))
         (conn (when cm (warp:connection-manager-get-connection cm)))
         (key-mgr (warp:component-system-get system :key-manager))
         (rpc-system (warp:component-system-get system :rpc-system))
         (worker-id (warp-worker-worker-id worker))
         ;; Retrieve launch tokens/IDs from environment variables set by the orchestrator.
         (launch-id (getenv (warp:env 'launch-id)))
         (master-token (getenv (warp:env 'launch-token)))
         (pool-name (getenv (warp:env 'worker-pool-name)))
         (data-to-sign (format "%s:%s" worker-id master-token)) ; Data to be signed for authentication
         (worker-sig (warp-worker--worker-sign-data worker data-to-sign))
         (pub-key (warp:key-manager-get-public-key-material key-mgr))
         ;; The inbox address is where the master can send RPCs back to this worker.
         (inbox-address (getenv (warp:env 'master-contact))) ; Typically from env
         (cs-id (warp-component-system-id system)))
    (unless conn (error "Cannot send ready signal: No active connection to leader."))
    (unless worker-sig (error "Failed to generate launch signature for worker ready signal."))
    (unless inbox-address (error "Worker inbox address not found in environment variables."))
    ...
    (loom:await
     (warp:protocol-send-worker-ready
      ...
      :starting ; Initial status indicating readiness
      launch-id master-token worker-sig pub-key ; Security-related parameters
      :inbox-address inbox-address
      :pool-name pool-name
      :origin-instance-id cs-id))))

(defun warp-worker--send-heartbeat (worker system)
  "Constructs and sends a heartbeat and metrics update to the current leader.
Called periodically by the `heartbeat-service` component, this function
ensures the control plane knows the worker is alive, its health status,
and its current workload. It uses the dynamically managed connection, so
heartbeats will automatically be sent to the new leader after a failover.
...
- (loom-promise or nil): A promise that resolves when the heartbeat is
  successfully sent, or `nil` if no connection to the leader is currently
  available.

Side Effects:
- Collects metrics from `system-monitor`, `transport`, `RPC-system`.
- Aggregates health status from `health-orchestrator`.
- Signs the heartbeat payload for authenticity.
- Sends an RPC request (`:send-heartbeat`) to the leader."
  (let* ((cm (warp:component-system-get system :connection-manager))
         (conn (when cm (warp:connection-manager-get-connection cm)))
         (cs-id (warp-component-system-id system)))
    (when conn
      (let* ((health (warp:component-system-get system :health-orchestrator))
             ...
             (monitor (warp:component-system-get system :system-monitor))
             (key-mgr (warp:component-system-get system :key-manager))
             ;; Safely get metrics, ignoring errors if components are not ready.
             ...
             ;; Await health report, as it aggregates async checks.
             (health-report (if health (loom:await (warp:health-orchestrator-get-aggregated-health health))
                                 '(:overall-status :unknown)))
             (rpc-metrics (warp:rpc-system-metrics (warp:component-system-get system :rpc-system)))
             (metrics
              (make-warp-worker-metrics
               :process-metrics proc-m
               :transport-metrics transport-m
               :active-request-count (gethash :active-request-count rpc-metrics 0)
               :total-requests-processed (gethash :total-requests-processed rpc-metrics 0) 
               :failed-request-count (gethash :failed-request-count rpc-metrics 0) 
               :average-request-duration (gethash :average-request-duration rpc-metrics 0.0) 
             ...
             ;; Sign the entire metrics payload for authenticity.
        ...
        (loom:await (warp:protocol-send-heartbeat
                     (warp:component-system-get system :rpc-system)
                     conn (warp-worker-worker-id worker)
                     (plist-get health-report :overall-status) ; Current worker status
                     (when registry (warp:service-registry-list-services registry)) ; Active services
                     "leader" ; Target ID for the heartbeat
                     :origin-instance-id cs-id ; Source instance ID for tracing
                     :metrics signed-payload))))) 
    ))
...
via the `key-manager` component.

Arguments:
- `worker` (warp-worker): The worker instance.
- `data` (string): The string data to sign.

Returns:
- (string): A Base64URL-encoded signature string.

Signals:
- `error`: If the `key-manager` is unavailable or no private key is loaded."
  (let* ((system (warp-worker-component-system worker))
         (key-mgr (warp:component-system-get system :key-manager)))
    (unless key-mgr
      (error "Key manager component not available for signing data."))
    (let ((priv-key-path (warp:key-manager-get-private-key-path key-mgr)))
      (unless priv-key-path
        (error "Worker private key not loaded for signing data."))
...
a full worker restart, significantly enhancing operational flexibility.
...
Returns:
- This function is a hook and does not have a meaningful return value.
...
- May change the global logging level via `warp:log-set-level`."
  (let* ((worker warp-worker--instance) ; Get the global singleton worker instance.
         (worker-id (warp-worker-worker-id worker))
         (system (warp-worker-component-system worker))
         (heartbeat-svc (warp:component-system-get system :heartbeat-service)))
    (setf (warp-worker-config worker) new-provision) ; Atomically update the worker's config.

    ;; If heartbeat interval changes, restart the heartbeat timer.
    ...
    ;; If log level changes, update the global log level.
...
This ID is unique and incorporates the system name, Emacs process ID,
and a random component, making it highly suitable for distributed environments.

Returns:
- (string): A unique string identifier for the worker (e.g.,
  `\"worker-hostname-12345-abcde123\"`)."
  (format "worker-%s-%d-%s"
          (or (system-name) "unknown") ; Hostname or "unknown"
          (emacs-pid)                 ; Emacs process ID
          (substring (secure-hash 'sha256 (random t)) 0 8))) ; Short random hash
...
to the master as part of its heartbeat.
...
- Registers multiple health checks with the `orchestrator` using
  ...
  periodically based on `health-check-interval` from the worker config."
  (let ((worker-id (warp-health-orchestrator-name orch)))
             ...
             (if (> cpu threshold)
                 (loom:rejected!
                  (format "CPU usage %.1f%% exceeds threshold %.1f%%"
                          cpu threshold))
               (loom:resolved! t))))
        :critical-p t ; Mark as critical check.
        :interval ,(worker-config-health-check-interval config))))
    ...
    ;; which affects worker responsiveness.
    (loom:await
     (warp:health-orchestrator-register-check
      orch
      `(:name "worker-event-queue-check"
        ...
        :check-fn
        ,(lambda (_)
           (let* ((es-metrics (warp:event-system-metrics es))
                  (depth (warp-event-system-metrics-events-in-queue es-metrics))
                  (max-size (warp-event-system-config-max-queue-size
                             (warp-event-system-config es)))
...
This serves as a basic liveness check for the worker, confirming that
it is responsive and its RPC server is operational.
...
worker's configured security policy, enabling live updates to worker capabilities.
If a `:service-name` is provided in the command args, the newly registered
command is also registered as a service endpoint with the worker's local
service registry.
...
Side Effects:
- Dynamically evaluates Lisp code within a sandbox.
- Modifies the `command-router` by adding a new route.
- If a `service-name` is provided, registers the new command's endpoint
  with the `service-registry`."
  (let* ((worker (plist-get context :worker))
         (system (warp-worker-component-system worker))
         (policy (warp:component-system-get system :security-policy))
         (router (warp:component-system-get system :command-router))
         (service-registry (warp:component-system-get system :service-registry)) 
    ...
    (braid! (warp:security-policy-execute-form policy form)
      (:then (lambda (handler-fn)
               ...
               (when (and s-name service-registry) 
                 (let* ((worker-id (warp-worker-worker-id worker))
                        ;; Assuming the worker's own main RPC inbox address is its service address
                        (inbox-address (getenv (warp:env 'master-contact))))
                   (warp:service-registry-register-service
                    service-registry s-name worker-id inbox-address
...
returns the results as a `warp-protocol-cluster-map-result`.

Arguments:
- `command` (warp-rpc-command): The incoming command object. Its `args`
  contain the function form and the data chunk to process.
- `context` (plist): The RPC request context, containing references to
  the worker's component system.
...
- Executes the provided function on the data chunk, potentially CPU-intensive."
  (let* ((worker (plist-get context :worker))
         (system (warp-worker-component-system worker))
         (policy (warp:component-system-get system :security-policy))
         (args (warp-rpc-command-args command))
...
Side Effects:
- Calls `warp:worker-stop` asynchronously, triggering the shutdown flow."
  (let ((worker (plist-get context :worker)))
    (warp:log! :info (warp-worker-worker-id worker)
...
crucial for key rotation in secure communication, maintaining a trusted
channel.
...
(defun warp-handler-execute-job (command context)
  "Handles the `:execute-job` RPC sent from the job processing loop.
This handler takes the job's payload (a Lisp form) and submits it to the
sandboxed `executor-pool` for safe execution. This entire function call
will be automatically wrapped in a server-side trace span by the global
middleware, providing tracing for job execution.

Arguments:
- `command` (warp-rpc-command): The incoming command, where `args` is the job payload.
- `context` (plist): The RPC context, containing a reference to the worker.

Returns:
- (loom-promise): A promise that resolves with the result of the job execution."
  (let* ((worker (plist-get context :worker))
         (system (warp-worker-component-system worker))
         (executor (warp:component-system-get system :executor-pool))
         (payload (warp-rpc-command-args command)))
    (warp:log! :debug (warp-worker-worker-id worker) "Executing job with payload: %S" payload)
    (warp:executor-pool-submit executor payload)))
...
- Attempts to drain active requests within a configured timeout.
- Calls `warp:component-system-stop` which stops all internal components
  (networking, RPC, etc.) in their reverse dependency order.
  ...
  (let* ((system (warp-worker-component-system worker))
    ...
    (let ((active-requests
      ...
      (while (and (> active-requests 0)
        ...
        ;; `accept-process-output` processes pending events.
      ...
      (when (> active-requests 0)
        (warp:log! :warn worker-id
                   "Shutdown timeout: %d requests still active. Proceeding."
                   active-requests)))
...
at which point it initiates its graceful shutdown.

Arguments:
- None.

Returns:
- This function does not return. It blocks indefinitely in an event loop.

Side Effects:
- Creates and starts the global `warp-worker` instance.
- Sets up `kill-emacs-hook` for graceful shutdown, linking Emacs exit
  to `warp:worker-stop`.
- Logs critical errors and exits the process on fatal startup failure,
  ensuring the process does not become a zombie."
  ...
  (condition-case err
      (let ((worker (warp:worker-create))) ; Config loaded from env vars by default
        ...
        (braid! (warp:worker-start worker)
          (:then (lambda (_)
                   (warp:log! :info (warp-worker-worker-id worker)
                              "Worker is running. Entering Emacs event loop.")
                   ;; This `while t (accept-process-output nil t))` keeps the
                   ;; Emacs process alive indefinitely, acting as the worker's
                   ;; main event loop, processing I/O and timers.
    ...
    (error
     ;; Catch any errors that occur even before the worker is fully created/started.
...
when creating a worker (e.g., in `warp:worker-create`), reducing boilerplate
and promoting consistent worker configurations across a cluster.
...
- `PROFILE-OPTIONS` (plist): A property list of default configuration
  ...
  `:enable-job-processing t`). These options will be merged with global
  defaults and any explicit options passed to `warp:worker-create`.

Returns:
- (symbol): The `NAME` of the profile.

Side Effects:
- Stores the profile definition in a global registry (`warp--worker-profiles`)."
  `(progn
...
- (plist or nil): The property list of configuration options for the
  profile, or `nil` if no profile with `PROFILE-NAME` is found."
